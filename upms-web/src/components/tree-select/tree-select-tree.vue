...
export default {
  ...
  watch: {
    data (newData, oldVal) {
      ...
      let selectArray = []
      this.selectedArray.forEach(userId => {
        if (userId in this.flatDic) selectArray.push(userId)
      })
      this.$emit('on-check', selectArray.map(userId => this.flatDic[userId]))
      if (this.expandAll) this.checkData(newData, false, true)
    },
    selectedArray (newVal, oldVal) {
      if (arrayEqual(newVal, oldVal)) return
      const filtedNewVal = newVal.filter(userId => userId in this.flatDic)
      this.$emit('on-check', filtedNewVal.map(userId => this.flatDic[userId]))
      this.$emit('on-clear')
      this.$nextTick(() => {
  ...
  methods: {
    ...
    checkData (data, emit, expandAll) {
      data.forEach(item => {
        if (this.selectedArray.includes(item.userId)) {
          this.$set(item, 'checked', true)
          this.checkedArray.push(item)
          if (emit) this.checkEmit(item.userId, item.title)
